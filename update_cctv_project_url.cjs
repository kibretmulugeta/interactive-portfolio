...
async function run() {
  try {
    ...
    const updatedProjects = projects.map(p => {
      if (p.title && (p.title.toLowerCase().includes('cctv') || p.title.toLowerCase().includes('face recognition'))) {
        console.log('Updating CCTV project:', p.title);
        return {
          ...p,
    ...
    const cctvProj = verify.projects.find(p => p.title && (p.title.toLowerCase().includes('cctv') || p.title.toLowerCase().includes('face recognition')));
    console.log('\nUpdated CCTV project:');
    console.log('Title:', cctvProj?.title);
    console.log('GitHub URL:', cctvProj?.githubUrl);
